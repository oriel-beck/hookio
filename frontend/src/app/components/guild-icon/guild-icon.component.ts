...
export class GuildIconComponent {
  ...
  guildShortName = computed(() => {
    const split = this.guild().name.split(' ');
    return split.at(0)?.at(0)?.toUpperCase()! + split.at(1)?.at(0)?.toUpperCase() || "";
  });

  guildIcon = computed(() => this.guild().iconUrl || defaultGuildIcon)
}
