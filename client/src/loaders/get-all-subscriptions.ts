...
export default function getAllSubscriptions({ params }: { params: Record<string, string | undefined> }) {
    ...
    return { subscriptions: fetch(`/api/subscriptions/${serverId}?subscriptionType=${Provider[provider as keyof typeof Provider]}&withCounts=true`).then((r) => r.json().catch(() => null)).catch(() => null) }
}
