...
export default function Guild({ icon, name, id }: Props) {
    ...
    return (
        <div onClick={() => onClick()} className="bg-gray-900 rounded-md cursor-pointer">
            <div className="flex flex-col space-y-3 p-5">
                <img
                    className="rounded-full border-gray-500 border-2"
                    src={icon === "fallback" ? fallback : icon}
                    alt={name}
                    width={200}
                    height={200}
                />
                <h2 className="text-white flex justify-center">{name}</h2>
            </div>
        </div>
    )
}